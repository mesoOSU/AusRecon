function [myEBSD] = OR_estimation(myEBSD,vis_m,vis_a,gen,num_Ors,plt_ksi)
    ...
    %%  Specific Paramaters for Generating Simulated Data
    if gen
        %num_martensite_orientations=1000;

        %set oriention relationship in terms of ksi angles
        ksi1=2.62;
        ksi2=8.98;
        ksi3=9.21;
        %[4.37,7.79,9.24]
        %Halfwidth of the scatter. Containts both variance in orientation
        %relationship, measurement error, and local orientation gradients in the
        %austenite
        noise_halfwidth=1.7*degree;

        %% generate synthetic data

        %generate random initial austenite orientation
        g_aus=rand(1,4);
        g_aus=g_aus/norm(g_aus);
        g_aus_quat=quaternion(g_aus(1),g_aus(2),g_aus(3),g_aus(4));
        g_aus_quat=idquaternion;

        g_aus_orient=orientation(g_aus_quat,CS_R,SS);

        %use a specific austenite orientation

        %g_aus_orient=orientation(inv(rotation('Euler',41.18*degree,47.98*degree,318.95*degree)),CS_A,SS);
        g_aus_orient=orientation((rotation('Euler',304.91*degree,43.73*degree,34.99*degree)),CS_R,SS);

        martensite=generate_simulated_data(g_aus_orient,[ksi1,ksi2,ksi3],noise_halfwidth,num_Ors,CS_T);
    else
        Ebsd = myEBSD.TruncEbsd;
...
%     ksi_initial=[0.00,9.74,9.74];       % NW
    ...
    %generate global odf
    odf_global_initial=calcODF(austenite,CS_R,SS,'halfwidth',halfwidth_in);

    %initial austenite guess is modal orientation of global pole figure
    [intensity,austenite_initial]=max(odf_global_initial);
