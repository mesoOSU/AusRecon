function [Packet] = PacketChar(myEBSD,Grains,gId,Packet)
%% Intialization
...
if Grains.grainId(gId,2) ~= TransID
    ...
    tmpInds{1} = find(Aus_Grain.orientations == AusOr);
    Aus_or(1) = rotation('Euler',AusOr.phi1,AusOr.Phi,AusOr.phi2,CS_T);
    ...
    WT_Len = max(max(tmpInds{1}),size(Mart_Grain,1));
...
[Packet] = BlockChar(myEBSD,Grains,gId,Packet);
