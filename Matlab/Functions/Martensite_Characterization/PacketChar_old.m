function [Packet] = PacketChar_old(myEBSD,Grains,gId,Packet,Twin)
    %% Intialization
...
%     GrnId = find(Grains.Ebsd.Parent.ID==GrainID);
    ...
    if Grains.grainId(gId,2) ~= TransID
        ...
        % Austin Addin: Okay, this is a stupid fix, but I think it works.
        % The packet error occurs every time the code tries to create a
        % twin that doesn't actually exist. I THINK(but didn't test this)
        % that the error also occurs everytime there is a repeated grainID
        % in the last column of grains.grainId, but that could just be a
        % coincidence, since twin errors are more likely to occur on grains
        % with multiple twins. (was true for AF96Example). The correct fix
        % would be to go back, find where twins are assigned, and get rid
        % of this "ghost twin". however, the band-aid fix is to just remove
        % empty tmpInds (which are the Indicies corresponding to the packet
        % being segmented)
        Aus_or  = Aus_or( cellfun('length',tmpInds) >0);
        tmpInds = tmpInds(cellfun('length',tmpInds) >0);
        count = length(tmpInds);
