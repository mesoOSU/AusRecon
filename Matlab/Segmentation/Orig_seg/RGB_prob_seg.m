...
% Image_full = imread('Image_Segmentation/Eric_Gammaprime/gamma_etch.png');
Image_full = imread('slice001.tiff');
% Image = im2double(Image_full(500:1250,500:1250)); % Don't delete, this is Eric's gammaprime truncation
Image = im2double(Image_full);
Image_vec = Image(:);
%%
...
mean_mat = min(data.mu);
mean_part = max(data.mu);
sigma_mat = min(sqrt(data.Sigma));
sigma_part = max(sqrt(data.Sigma));
%%
...
% reshape([1:len(Image_vec),sizeI(1),sizeI(2))
index = zeros(sizeI);
counter = 1;
for j = 1:sizeI(2)
    for i = 1:sizeI(1)
...
figure
seg_I = zeros(length(Image_vec),1);
seg_I(ctcopy,1)=1;
